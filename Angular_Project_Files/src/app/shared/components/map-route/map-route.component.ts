...
export class MapRouteComponent implements OnInit {
  map!: mapboxgl.Map;
  directionsService: string = environment.mapbox.directionService;
  ...
  checkIfInView(){
    ...
    const srcView = bounds?.contains([this.sourceLng, this.sourceLat]);
    const destView = bounds?.contains([this.destLng, this.destLat]);
