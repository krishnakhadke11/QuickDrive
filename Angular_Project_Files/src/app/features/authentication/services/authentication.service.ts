...
export class AuthenticationService {
  ...
  customerSignup(data: CustomerRequest): Observable<Customer> {
    return this.http
      .post<Customer>(this.base_url + `auth/customers/signup`, data)
      .pipe(
        catchError((err) => {
          if (err.status === 400) {
  ...
  driverSignup(data: DriverRequest): Observable<Driver> {
    return this.http
      .post<Driver>(this.base_url + `auth/drivers/signup`, data)
      .pipe(
        catchError((err) => {
          if (err.status === 400) {
  ...
  login(data: any, role: Role) {
    return this.http
      .post<AuthResponse>(this.base_url + `auth/signin`, data)
      .pipe(
        catchError((err) => {
          if (err.status === 400) {
            return throwError(() => 'Invalid Credentials');
        ...
        tap((res) => {
          ...
          const user = new User(res.token, res.refreshToken, res.role);

          this.notif.showSuccess('Login Successfull');
          localStorage.setItem(
            'user',
            JSON.stringify(
              new User(res.token, res.refreshToken, this.encryptData(res.role))
            )
          );
          this.user.next(user);
        })
  ...
  refreshAccessToken(): Observable<AuthResponse> {
    ...
    return this.http
      .post<AuthResponse>(this.base_url + `auth/refresh-token`, {
        token: this.user.getValue()?.refreshToken,
      })
      .pipe(
        tap((res) => {
        ...
        catchError((error) => {
          ...
          this.notif.showError('Session has been expired');
          return of();
        })
      );
  }
